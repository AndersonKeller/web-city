...
import { ANIMBIT, BIT_MASK, POWERBIT, ZONEBIT } from "../tiles/tileFlags";
import { tileHistoryController } from "../tiles/tileHistory-ts";
import { tileUtilsController } from "../tiles/tileUtils-ts";
import { LASTTINYEXP, LIGHTNINGBOLT, TILE_COUNT, TILE_INVALID } from "../tiles/tileValues";
export type iAnimationManager = typeof animationManagerController;
export const animationManagerController = {
  _map: null as GameMap,
  animationPeriod: 0,
  lastAnimation: new Date(1970, 1, 1),
  lastBlink: new Date(1970, 1, 1),
  blinkPeriod: 0,
  shouldBlink: false,
  _lastPainted: null,
  _currentPainted: null,
  _data: [],

  init(map: GameMap, animationPeriod?: number, blinkPeriod?: number) {
  ...
  inSequence(tileValue: number, lastValue: number) {
  ...
  getTiles(tileValues: number[], offsetX: number, offsetY: number, xBound: number, yBound: number, isPaused: boolean) {
    ...
    if (d.getTime() - this.lastBlink.getTime() > this.blinkPeriod) {
      shouldBlink = this.shouldBlink = !this.shouldBlink;
      this.lastBlink = d;
    }

    if (!isPaused) {
      if (d.getTime() - this.lastAnimation.getTime() > this.animationPeriod) {
        shouldChangeAnimation = true;
        this.lastAnimation = d;
      }
    }

    const newPainted = this._currentPainted === null ? new tileHistoryController() : this._currentPainted;
      ...
      for (let x = 0; x < xBound; x++) {
        ...
        if (shouldChangeAnimation) {
          ...
          if (last && this.inSequence(tileValue, last)) {
            ...
            if (last === LASTTINYEXP) {
              this._map.setTo(mapX, mapY, tileUtilsController.randomRubble());
    ...
    // console.log(temp,"temp?w")
    this._currentPainted = temp;
  },
  registerSingleAnimation(arr: number[]) {
